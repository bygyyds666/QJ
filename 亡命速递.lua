...
local function showTimeWindow(mode)
    ...
    local conn; conn = g:GetPropertyChangedSignal("Parent"):Connect(function()
        if not g.Parent then conn:Disconnect() end
    end)
    task.spawn(function()
        while f.Parent do
            local now = os.date("*t")
...
-- 安全创建UI组件的辅助函数
local function safeAddComponent(creator, componentType, ...)
    local args = {...}
    local success, err = pcall(function()
        creator(unpack(args))
    end)
    if not success then
        warn("创建 " .. componentType .. " 失败: " .. tostring(err))
    end
end
...
if MainTab then
    MainTab:Section({Title = "核心功能"})
    safeAddComponent(function() MainTab:Button({
        Title = "召唤飞行脚本",
        Icon = "plane",
        Callback = function()
            local s, e = pcall(loadFlightScript)
            if s then Notify("飞行脚本已加载") else Notify("飞行脚本加载失败: "..tostring(e)) end
        end
    }) end, "按钮")

    safeAddComponent(function() MainTab:Toggle({
        Title = "自动翻译界面",
        Icon = "translate",
        Default = false,
        Callback = function(v)
    ...
    safeAddComponent(function() MainTab:Slider({
        Title = "移动速度",
        Icon = "gauge",
        Default = 16,
        Min = 16,
    ...
    safeAddComponent(function() MainTab:Button({
        Title = "显示现在时间",
        Icon = "clock",
        Callback = function() showTimeWindow("now") end
    }) end, "按钮")

    safeAddComponent(function() MainTab:Button({
        Title = "显示已用时长",
        Icon = "hourglass",
        Callback = function() showTimeWindow("used") end
    }) end, "按钮")

    safeAddComponent(function() MainTab:Button({
        Title = "我全都要 (时间和时长)",
        Icon = "timer",
        Callback = function() showTimeWindow("both") end
    }) end, "按钮")
...
if ESPTab then
    ESPTab:Section({Title = "透视选项"})

    safeAddComponent(function() ESPTab:Toggle({
        Title = "透视物资 (显示价格)",
        Icon = "dollar-sign",
        Default = false,
        Callback = function(v)
    ...
    safeAddComponent(function() ESPTab:Slider({
        Title = "最低价格过滤",
        Icon = "filter",
        Default = 0,
        Min = 0,
...
if SettingsTab then
    SettingsTab:Section({Title = "全局"})
    safeAddComponent(function() SettingsTab:Button({
        Title = "卸载脚本",
        Icon = "trash",
        Callback = function()
